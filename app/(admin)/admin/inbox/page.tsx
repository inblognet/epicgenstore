        ...
        {messages.length === 0 ? (
          ...
          messages.map((msg) => (
            <div key={msg.id} className="bg-surface-card border border-zinc-800/50 rounded-2xl p-6 shadow-xl relative group transition-colors duration-300">
              ...
              {/* 🚀 FIXED: Added z-10 and adjusted positioning so it's perfectly clickable */}
              <form action={async () => { "use server"; await deleteContactMessage(msg.id); }} className="absolute top-4 right-4 md:top-5 md:right-5 z-10 opacity-0 group-hover:opacity-100 transition-opacity">
                {/* 🚀 FIXED: Added padding and hover background for a better hit-box */}
                <button type="submit" title="Delete Message" className="text-zinc-500 hover:text-red-500 hover:bg-red-500/10 p-2 rounded-lg transition-colors duration-300">
                  <Trash2 className="w-5 h-5" />
                </button>
              </form>

              {/* 🚀 FIXED: Added pr-16 (padding-right) to ensure the text never slides under the absolute button */}
              <div className="flex flex-wrap gap-x-8 gap-y-4 mb-4 pb-4 border-b border-zinc-800/50 pr-16">
                <div>
