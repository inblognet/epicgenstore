...
// cspell:ignore Poya Delpe Ragama
import { prisma } from "@/lib/prisma";
import { defaultContactData } from "@/components/admin/contact-page-editor";
import { Phone, MapPin, Mail, Headset, Laptop, Monitor, Wrench, Database, ArrowUpCircle, ShieldAlert, Cpu } from "lucide-react";
...
export default async function ContactPage() {
  ...
  // 🚀 FIXED: Safely typed the hero object to remove the `any` error
  const data = (rawData && rawData.hero) ? {
    ...
    hero: {
      ...
      ...rawData.hero,
      highlightedText: (rawData.hero as Record<string, string>)?.highlightedText || "MSK COMPUTERS"
    }
  } : defaultContactData;
    ...
    <div className="bg-surface-bg min-h-screen text-zinc-50 font-sans pb-24 transition-colors duration-300">
      ...
      <section className="container mx-auto px-4 pt-20 pb-16 max-w-4xl text-center">
        ...
        <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight mb-6">
          {data.hero.title} <span className="text-brand transition-colors duration-300">{data.hero.highlightedText}</span>
        </h1>
            ...
            <div className="bg-surface-card border border-zinc-800/50 rounded-3xl p-8 transition-colors duration-300">
              ...
              <div className="space-y-6">
                <div className="flex gap-4 items-start">
                  ...
                  <div>
                    <h4 className="text-white font-bold text-sm mb-1">{data.hero.highlightedText}</h4>
