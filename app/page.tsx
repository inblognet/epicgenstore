// app/page.tsx
// cspell:ignore epicgenstore wishlisted KOKO Islandwide
import { Product, Category, Carousel } from "@prisma/client";
...
type StoreCarousel = Carousel & {
  category?: (Category & { products: StoreProduct[] }) | null;
};

type CustomerExperienceImage = {
  id: string;
  order: number;
  createdAt: Date;
  url: string;
...
interface HomepageCacheData {
  promoProducts: StoreProduct[];
  newArrivalProducts: StoreProduct[];
  activeCarousels: StoreCarousel[];
  experienceImages: CustomerExperienceImage[];
  categories: Category[];
}
...
export default async function HomePage() {
  ...
  // ==========================================
  const CACHE_KEY = "epicgenstore:homepage:data";

  const cachedData = await redis.get<HomepageCacheData>(CACHE_KEY);

  let promoProducts: StoreProduct[];
  let newArrivalProducts: StoreProduct[];
  let activeCarousels: StoreCarousel[];
  let experienceImages: CustomerExperienceImage[];
  let categories: Category[];

  if (cachedData) {
    console.log("⚡ [REDIS] CACHE HIT: Loading homepage from memory");
    promoProducts = cachedData.promoProducts;
    newArrivalProducts = cachedData.newArrivalProducts;
    activeCarousels = cachedData.activeCarousels;
    experienceImages = cachedData.experienceImages;
    categories = cachedData.categories;
  } else {
    console.log("🐌 [REDIS] CACHE MISS: Querying PostgreSQL database...");

    const [fetchedPromo, fetchedNew, fetchedCarousels, fetchedExp, fetchedCats] = await Promise.all([
    ...
    promoProducts = fetchedPromo as StoreProduct[];
    newArrivalProducts = fetchedNew as StoreProduct[];
    activeCarousels = fetchedCarousels as unknown as StoreCarousel[];
    experienceImages = fetchedExp as CustomerExperienceImage[];
    categories = fetchedCats as Category[];
    ...
    <div className="min-h-screen bg-surface-bg text-theme-main font-sans transition-colors duration-300 overflow-hidden">
      ...
      <section className="py-12 md:py-16 px-4 container mx-auto max-w-7xl">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6 items-stretch">
          {newArrivalProducts.map((product: StoreProduct, index: number) => {
      ...
      <section className="py-16 px-4 bg-surface-card/10 border-y border-theme-border transition-colors duration-300">
        <div className="container mx-auto max-w-7xl">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6 mb-8 items-stretch">
            {promoProducts.map((product: StoreProduct, index: number) => {
                  ...
                  <div className="h-full bg-transparent border border-theme-border rounded-xl md:rounded-2xl overflow-hidden hover:border-brand/50 transition-all group flex flex-col shadow-sm hover:shadow-2xl hover:-translate-y-2 duration-500">
                    <Link href={`/product/${product.slug}`} className="block relative aspect-[4/5] bg-transparent overflow-hidden border-b border-theme-border shrink-0 transition-colors duration-300 group/img">
                      {product.imageUrl ? (
                        <>
                          {/* eslint-disable-next-line @next/next/no-img-element */}
                          <img src={product.imageUrl} alt={product.name} className={`absolute inset-0 p-3 md:p-6 object-contain w-full h-full transform group-hover/img:scale-110 transition-transform duration-500 drop-shadow-md ${hasHoverImage ? 'group-hover/img:opacity-0' : ''}`} />
                          {hasHoverImage && (
                            /* eslint-disable-next-line @next/next/no-img-element */
                            <img src={hoverImageUrl!} alt={`${product.name} alternate view`} className="absolute inset-0 p-3 md:p-6 object-contain w-full h-full transform scale-95 opacity-0 group-hover/img:scale-110 group-hover/img:opacity-100 transition-all duration-500 drop-shadow-md" />
                          )}
                        </>
                      ) : (
                        <div className="w-full h-full flex items-center justify-center text-theme-muted text-[10px] md:text-sm font-black tracking-widest uppercase text-center absolute inset-0">No Image</div>
