    ...
    <div className="min-h-screen bg-surface-bg text-theme-main font-sans pb-24 transition-colors duration-300">
      ...
      <section className="py-20 px-4 container mx-auto max-w-6xl">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
          ...
          // FIXED: Removed the unused 'i' variable from the map
          ].map((feature) => (
        ...
        <div className="container mx-auto max-w-7xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
            {promoProducts.map((product) => (
              <div key={product.id} className="bg-surface-bg border border-theme-border rounded-2xl overflow-hidden hover:border-brand/50 transition-all group flex flex-col shadow-lg duration-300">
                ...
                <div className="relative aspect-[4/3] bg-surface-card/50 overflow-hidden border-b border-theme-border group/img">
                      ...
                      <>
                        ...
                        {product.images && product.images.length > 0 && (
                          ...
                          <img
                            ...
                            className="absolute inset-0 p-6 object-contain w-full h-full transform scale-95 opacity-0 group-hover/img:scale-110 group-hover/img:opacity-100 transition-all duration-500 drop-shadow-2xl"
                          />
                        )}
                      </>
                    ) : (
                  ...
                  <div className="absolute top-4 left-4 flex flex-col gap-2 z-10 pointer-events-none">
                    ...
                    <span className="bg-red-600 text-white text-[10px] font-black px-2 py-1 rounded shadow-md tracking-wider animate-pulse">
                      PROMO
                    </span>
                  </div>
                    ...
                    <div className="font-black text-lg">
                      {product.salePrice ? (
                        <div className="flex flex-col">
                          <span className="text-xs text-theme-muted line-through">LKR {Number(product.price).toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
          ...
          <div className="flex justify-center">
            <Link
              ...
              className="border-2 border-brand text-brand hover:bg-brand hover:text-black font-black text-sm uppercase tracking-widest py-3 px-8 rounded-full transition-all shadow-lg shadow-brand/20 hover:shadow-xl hover:shadow-brand/40 duration-300"
            >
              View All Promotions
            </Link>
          </div>
...
function BannerCategoryLayout({
  ...
  return (
    <section key={id} className="py-16 px-4 container mx-auto max-w-7xl">
      <div className="flex items-center justify-between mb-12 border-b-4 border-theme-border pb-6 transition-colors duration-300">
        <h2 className="text-4xl md:text-5xl font-black text-theme-main tracking-tighter transition-colors duration-300">
          {title}
        </h2>
        <Link href={`/products?category=${categorySlug}`} className="text-theme-muted hover:text-brand font-black text-[11px] uppercase tracking-wider flex items-center gap-1.5 transition-colors duration-300 group">
          View All <ChevronRight className="w-4 h-4 text-theme-muted group-hover:text-brand transition-colors"/>
        </Link>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 items-start">
        {/* Promotional Banner Image Card */}
        <div className="aspect-[3/4] rounded-2xl border border-theme-border transition-colors duration-300 relative overflow-hidden flex items-end p-8 flex-col shadow-2xl">
          <div className="absolute inset-0 z-0">
            {/* eslint-disable-next-line @next/next/no-img-element */}
          ...
          <div className="relative z-10 text-center w-full mt-auto space-y-4">
            <span className="text-[10px] font-black text-brand uppercase tracking-[0.2em]">{banner.imageSubtitle || "Epic Deals"}</span>
            <h1 className="text-4xl font-black mb-1 leading-tight text-white">{banner.imageTitle || title}</h1>
            <Button asChild className="w-full bg-theme-main text-black hover:bg-brand transition-colors duration-300 font-bold uppercase tracking-widest text-[11px] py-6 rounded-xl">
              ...
              <div className="mt-auto pt-4 flex flex-col gap-4">
                <div className="font-black text-lg">
                  {product.onSale && product.salePrice ? (
                    <div className="flex flex-col">
                      <span className="text-xs text-theme-muted line-through">LKR {product.price.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
        ...
        {products.length === 0 && (
          <div className="lg:col-span-3 text-center py-12 border border-theme-border border-dashed rounded-2xl bg-surface-card/30">
